...
class CompoundClassifier:
    ...
    def _ensure_vector(self, x):
        """Ensure input is a numpy 1D array."""
        if isinstance(x, (list, tuple)):
            return np.array(x)
        if isinstance(x, dict):
            return np.array(list(x.values()))
        if hasattr(x, "values"):  # pandas row
            return x.values
        return x

    def predict_single(self, x):
        """
        x = 1D Array ohne molekuelname (gleiche Features wie main.csv)
        """
        ...
        # Predict by checking neighbors within eps
        # (Sklearn bietet kein predict(), also machen wir es manuell)
