...
def evaluate_single_row(row, cc):
    """
    Klassifikation eines einzelnen Datenpunktes (Pandas Series)
    """
    features = row.drop(['molekuelname', 'category']).values
    name = row['molekuelname']
    true_cat = row['category']

    # Warnungen temporär unterdrücken
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", UserWarning)
        pred_label = cc.predict_single(features)
        
    # korrekt/falsch
    if (true_cat == 1 and pred_label == "outlier") or (true_cat in [2,3,4] and pred_label != "outlier"):
        correct = False
    else:
        correct = True
...
def evaluate_all(plot_first2_features=True):
    """
    Bewertung aller Samples in novel.csv
    """
    cc = load_cc()
    df = pd.read_csv(NOVEL_CSV)

    category_errors = {1: [], 2: [], 3: [], 4: []}
    n_total = len(df)
    ...
    if plot_first2_features and X_plot:
        ...
        plt.xlim(0, 1)   # Hinzugefügt: feste X-Skala
        plt.ylim(0, 1)   # Hinzugefügt: feste Y-Skala
        plt.show()
