...
def auto_dbscan(
    ...
    """
    Führt einen automatischen DBSCAN-Fit aus basierend auf:
      - sqrt(N/d) für min_samples
      - KneeLocator für eps
      - min_inlier_ratio als Fallback-eps

    Parameter
    ---------
    data_csv : str
        Pfad zur CSV: vectors.csv.
    model_file : str
        Zielpfad, unter dem das Modell gespeichert wird.
    metric : str
        Distanzmetrik, default 'jensenshannon'.
    min_inlier_ratio : float
        Minimale Inlier-Rate, um knee-eps zu akzeptieren.
    verbose : bool
        Ob Statusmeldungen ausgegeben werden.

    Returns
    -------
    dict
        Enthält DBSCAN-Modell + Parameter.
    """
